...
function convertTimeForDTlocal(time: string){
...
const fetchTournaments = () => {
  $fetch('/api/tournaments', {
    ...
    .then((res: any) => {
      if (res.tournament) {
        const now = new Date();
        data.value = { ...res.tournament,
          begin_time: res.tournament.begin_time ? convertTimeForDTlocal(res.tournament.begin_time) : '',
          end_time: res.tournament.end_time ? convertTimeForDTlocal(res.tournament.end_time) : ''
        }
      }
    })
    .catch((err) => {
...
const submitCreate = async () => {
  ...
  if (Date.parse(adjustedData.begin_time) && Date.parse(adjustedData.end_time) && adjustedData.begin_time >= adjustedData.end_time) {
    const toastHTML = `<strong>Update Failure</strong>\nEnd time must be after begin time`;
...
<template>
  <div class="flex flex-col gap-2">
    ...
    <form class="flex flex-col gap-2" @submit.prevent="submitCreate">
      ...
      <Button class="mt-4" :disabled="isSubmitting">Save</Button>
    </form>
  </div>
</template>